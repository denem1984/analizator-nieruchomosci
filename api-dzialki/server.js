...
function pngNonTransparentPixels(buf){
  ...
  const stride=width*4;let nonTransparent=0,firstHit=null;
  for(let y=0;y<height;y++){
    const rowStart=y*(stride+1)+1;
    for(let x=0;x<width;x++){
      if(raw[rowStart+x*4+3]>0){
        nonTransparent++;
...
async function mpzpInfo(reqUrl,res){
  ...
  if(!isFinite(lat)||!isFinite(lon))return send(res,400,'application/json; charset=utf-8',JSON.stringify({error:'Brak lat/lon'}));
  const d=0.0006;
  ...
  const url='https://rejestr-urbanistyczny.gov.pl/uslugi-sieciowe/wms-mpzp/ows?SERVICE=WMS&VERSION=1.3.0&REQUEST=GetFeatureInfo'
    ...
    +'&STYLES=,,&CRS=EPSG:4326&BBOX='+bbox+'&WIDTH=101&HEIGHT=101&I=50&J=50'
    +'&INFO_FORMAT=application%2Fjson&FEATURE_COUNT=5&FORMAT=image/png&TRANSPARENT=TRUE';
  try{
    const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),15000);
    const r=await fetch(url,{signal:controller.signal,headers:{'User-Agent':'Mozilla/5.0 (compatible; MAPA/1.0)'}});
    clearTimeout(timer);
    const ct=r.headers.get('content-type')||'';
    if(!ct.includes('json'))return send(res,200,'application/json; charset=utf-8',JSON.stringify({found:false}));
    const data=await r.json();
    const plany=(data.features||[]).map(f=>({
      tytul:f.properties&&f.properties['Tytuł'],
      status:f.properties&&f.properties['Status'],
      identyfikator:f.properties&&f.properties['Identyfikator'],
      wersjaOd:f.properties&&f.properties['Wersja obowiązuje od'],
      link:f.properties&&f.properties['Link do Rejestru Urbanistycznego']
    }));
    return send(res,200,'application/json; charset=utf-8',JSON.stringify({found:plany.length>0,plany}));
  }catch(e){
    return send(res,200,'application/json; charset=utf-8',JSON.stringify({found:false,error:e.message}));
  }
}


async function probeRuPlanDetails(res){
  const result={};
  const candidates=[
    'https://rejestr-urbanistyczny.gov.pl/published/details/mpzp/590',
    'https://rejestr-urbanistyczny.gov.pl/api/published/details/mpzp/590',
    'https://rejestr-urbanistyczny.gov.pl/api/plans/mpzp/590',
    'https://rejestr-urbanistyczny.gov.pl/api/app/mpzp/590'
  ];
  for(const url of candidates){
    try{
      const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),12000);
      const r=await fetch(url,{signal:controller.signal,headers:{'User-Agent':'Mozilla/5.0 (compatible; MAPA-probe/1.0)','Accept':'application/json,text/html,*/*'}});
      clearTimeout(timer);
      const ct=r.headers.get('content-type')||'';
      const text=await r.text();
      result[url]={status:r.status,contentType:ct,length:text.length,
        apiHints:Array.from(new Set(Array.from(text.matchAll(/["'](\/api\/[a-zA-Z0-9_\-\/]+)["']/g)).map(m=>m[1]))).slice(0,20),
        preview:ct.includes('json')?text.slice(0,2000):undefined};
    }catch(e){result[url]={error:e.name==='AbortError'?'Timeout':e.message}}
  }
...
async function probeRuFeatureInfo(res){
  ...
  const result={};
  const mapUrl='https://rejestr-urbanistyczny.gov.pl/uslugi-sieciowe/wms-mpzp/ows?SERVICE=WMS&VERSION=1.3.0&REQUEST=GetMap'
    +'&LAYERS=APP.MPZP.PrawnieWiazacyLubRealizowany&STYLES=&CRS=EPSG:2180&BBOX='+bbox
    +'&WIDTH=688&HEIGHT=607&FORMAT=image/png&TRANSPARENT=TRUE';
  let hit=null;
  try{
    const rMap=await fetch(mapUrl,{headers:{'User-Agent':'Mozilla/5.0 (compatible; MAPA-probe/1.0)'}});
    const buf=Buffer.from(await rMap.arrayBuffer());
    const px=pngNonTransparentPixels(buf);
    result.pixelScan=px;
    hit=px.firstHit;
  }catch(e){result.pixelScan_error=e.message}
  if(hit){
    const infoUrl='https://rejestr-urbanistyczny.gov.pl/uslugi-sieciowe/wms-mpzp/ows?SERVICE=WMS&VERSION=1.3.0&REQUEST=GetFeatureInfo'
      +'&LAYERS=APP.MPZP.PrawnieWiazacyLubRealizowany&QUERY_LAYERS=APP.MPZP.PrawnieWiazacyLubRealizowany&STYLES=&CRS=EPSG:2180&BBOX='+bbox
      +'&WIDTH=688&HEIGHT=607&I='+hit.x+'&J='+hit.y+'&INFO_FORMAT=application%2Fjson&FEATURE_COUNT=5&FORMAT=image/png&TRANSPARENT=TRUE';
    result.getFeatureInfoUrl=infoUrl;
    try{
      const rInfo=await fetch(infoUrl,{headers:{'User-Agent':'Mozilla/5.0 (compatible; MAPA-probe/1.0)'}});
      const text=await rInfo.text();
      result.status=rInfo.status;result.contentType=rInfo.headers.get('content-type');result.body=text.slice(0,4000);
    }catch(e){result.getFeatureInfo_error=e.message}
  }else{
    result.note='nie znaleziono nieprzezroczystego piksela w obrazie - nie mam gdzie pytać';
  }
  return send(res,200,'application/json; charset=utf-8',JSON.stringify(result,null,2));
}
