...
async function getHtml(){
 ...
 const r=await fetch(SOURCE,{headers:{'Cache-Control':'no-cache','User-Agent':'MAPA integration/5.0'}});
 if(!r.ok)throw new Error('GitHub source HTTP '+r.status);
 let html=await r.text();
 if(!WFS_RE.test(html))throw new Error('Nie znaleziono deklaracji parcelWfs.');
 ...
 console.log('INTEGRATION_SOURCE_OK',JSON.stringify({wfsReplaced:html.includes('const parcelWfs=location.origin+"/api/wfs"'),font16:html.includes('Math.min(16,10+(z-13)*.72)'),length:html.length}));
 return html;
}
...
http.createServer(async(req,res)=>{try{const u=new URL(req.url,'http://localhost');console.log('INTEGRATION_REQUEST',req.method,u.pathname,u.search);
 ...
 if(u.pathname==='/health')return send(res,200,'application/json; charset=utf-8',JSON.stringify({ok:true,service:'MAPA integration WFS v5'}));
 if(u.pathname==='/source-check'){const html=await getHtml();return send(res,200,'application/json; charset=utf-8',JSON.stringify({ok:true,wfsProxy:html.includes('const parcelWfs=location.origin+"/api/wfs"'),fontMax16:html.includes('Math.min(16,10+(z-13)*.72)'),htmlLength:html.length,cachedAt:new Date(cachedAt).toISOString()}));}
...
}catch(e){console.error('INTEGRATION_ERROR',e);return send(res,500,'text/plain; charset=utf-8','Błąd integracji: '+e.message)}}).listen(PORT,'0.0.0.0',()=>console.log('MAPA integration WFS v5 listening on '+PORT));